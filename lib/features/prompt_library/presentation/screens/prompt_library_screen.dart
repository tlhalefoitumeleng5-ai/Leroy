...
class PromptLibraryScreen extends ConsumerWidget {
  ...
  Future<void> _usePrompt(
    ...
    final session = await ref.read(chatListProvider.notifier).createSession(
          title: 'From prompt library',
        );
    if (session == null) {
      if (context.mounted) {
        AppSnackBar.show(context, 'Could not start chat', isError: true);
      }
      return;
    }
    if (!context.mounted) return;
    final encoded = Uri.encodeComponent(content);
    context.push('/chat/${session.id}?prompt=$encoded');
  }
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Material(
                            ...
                            child: InkWell(
                              borderRadius: BorderRadius.circular(20),
                              onTap: () => _usePrompt(context, ref, prompt.content),
                              child: Container(
                                padding: const EdgeInsets.all(16),
